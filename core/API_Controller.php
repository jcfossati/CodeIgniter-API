...
/*
 * API_Controller.php
 * PHP Class to write restful apis using CodeIgniter
 */

class API_Controller extends CI_Controller {

    //Config to load
    private $configFile = "rest";
    ...
    public function __construct() {
        ...
        $this->versionCheck();
        $this->load->config($this->configFile);
    }

    /**
     * Checks supported version of PHP and CodeIgniter
     * @throws Exception
     */
    private function versionCheck() {
        //Check supported PHP version
        if (is_php('5.4') === FALSE) {
            throw new Exception('Using PHP v' . PHP_VERSION
            . ', though PHP v5.4 or greater is required');
        }
    ...
    public function _remap($object_called, $arguments) {
        ...
        if ($this->config->item('force_https') && is_https() === FALSE) {
            $responseObject = $this->response(true, 0, ["Allowed over HTTPS"]);
            $this->finalResponse($responseObject, 405);
        }
        $controller_method = $this->input->method() . '_' . $object_called;
        if (method_exists($this, $controller_method) === FALSE) {
            $responseObject = $this->response(true, 0, ["Method dose not exist"]);
            $this->finalResponse($responseObject, 404);
        }

        //Validate request
        $this->validateRequest($this, $controller_method);
        // Call the controller method and passed arguments
        try {
            $finalResponse = call_user_func_array([$this, $controller_method], $arguments);
            $this->finalResponse($finalResponse, 200);
        } catch (Exception $ex) {
            $message = [
                'classname' => get_class($ex),
                'message' => $ex->getMessage()
            ];
            $responseObject = $this->responseObject(true, 0, $message);
            $this->finalResponse($responseObject, 500);
        }
    }
    
    /**
     * Validate all applied validations
     * @param string $class
     * @param string $method
     */
    private function validateRequest($class, $method) {
        $annotation = new Annotation();
        $annotation->set($class, $method);
        $this->validateAuth($annotation->get("auth"));
        //query, raw two type of parameters allowed
        $this->validateQuery($annotation->get("query"));
        $this->validateRaw($annotation->get("raw"));
    }

    /**
     * Validate all auth methods and return error
     * if validation fails
     * @param Array $authValidations
     */
    private function validateAuth($authValidations) {
        foreach ($authValidations as $key => $val) {

            if (!file_exists(APPPATH . "models/$key.php")) {
                $response = $this->response(true, 0, ["Invalid auth method [$key]"]);
                $this->finalResponse($response, 500);
            }
            $this->load->model($key);
            $module_name = explode("/", $key);

            if (!method_exists($this->{end($module_name)}, "validate")) {
                $response = $this->response(true, 0, ["Auth method must implement [AuthInterface]"]);
                $this->finalResponse($response, 500);
            }

            $is_valid = $this->{end($module_name)}->validate();
            if ($is_valid !== true) {
                $response = $this->response(true, 0, ["Invalid or empty header [$val]"]);
                $this->finalResponse($response, 200);
            }
        }
     ...
     * @return boolean
     */
    private function validateQuery($validators) {
        $validation = new InputValidation();
        $valid = $validation->is_valid($this->input->get(), $validators);
        if ($valid !== true) {
            $response = $this->response(true, 0, $valid);
            $this->finalResponse($response, 200);
        }
        return true;
    }
    ...
    /**
     * Validate raw parameters parameters
     * @param array $validators
     * @return boolean
     */
    private function validateRaw($validators) {
        $validation = new InputValidation();
        $valid = $validation->is_valid($this->input->input_stream(), $validators);
        if ($valid !== true) {
            $response = $this->response(true, 0, $valid);
            $this->finalResponse($response, 200);
        }
     ...
     * @return Array
     */
    public function response($error, $status, $message) {
        return [
            'error' => $error,
            'status_code' => $status,
            'message' => $message
     ...
     * @param int $status
     */
    private function finalResponse($array, $status) {
        if (!is_array($array) || !is_int($status)) {
            $status = 500;
            $array = $this->response(true, 0, ['Return value is not valid']);
        }
        http_response_code($status);
        header("Content-Type: application/json");
        foreach ($this->_response_headers as $key => $val) {
            header("$key: $val");
        }
        $array['_process_time'] = $this->getProcessTime();
    ...
    /**
     * Get Request parsed raw data
     * @param String $index
     * @return Mixed
     */
    public function raw($index = null){
        return $this->input->input_stream($index);
    }
}

/**
 * Class annotation for parsing 
 * method annotation data
 */
class Annotation {

    private $all_annotation;
    private $class;
    private $method;
    private $annotation_regex = '/@(\w+)(?:\s*(?:\(\s*)?(.*?)(?:\s*\))?)??\s*(?:\n|\*\/)/';
    private $parameter_regex = '/(\w+)\s*=\s*(\[[^\]]*\]|"[^"]*"|[^,)]*)\s*(?:,|$)/';
     ...
     * @param string $method
     */
    private function load($class, $method) {
        $reflaction = new \ReflectionMethod($class, $method);
        $document = $reflaction->getDocComment();
        $this->all_annotation = $this->parse_document($document);
    }
     ...
     * @return array
     */
    private function parse_document($document) {
        $has_annotations = preg_match_all($this->annotation_regex, $document, $matches, PREG_SET_ORDER);
        if (!$has_annotations) {
            return array();
        }
        $annotations = array();
        foreach ($matches AS $anno) {
            ...
            if (isset($anno[2])) {
                //var_dump($anno[2]);
                $has_params = preg_match_all($this->parameter_regex, $anno[2], $params, PREG_SET_ORDER);

                if ($has_params) {
                    $val = array();
                    foreach ($params AS $param) {
                        $val[$param[1]] = $this->parse_value($param[2]);
                    }
                } else {
                    $val = (trim($anno[2]) == '') ? true : $this->parse_value($anno[2]);
                }
            }
            if (isset($annotations[$annoName])) {
                $annotations[$annoName] = is_array($annotations[$annoName]) ? $annotations[$annoName] : [$annotations[$annoName]];
    ...
    /**
     * Sets class and method for annotation
     * @param string $class
     * @param string $method
     */
    public function set($class, $method) {
        $this->class = $class;
        $this->method = $method;
    }

    /**
     * Get annotations for given key
     * @param string $key
     * $return Array
     */
    public function get($key) {

        $this->load($this->class, $this->method);
        $params = [];
        if (isset($this->all_annotation[$key]) && count($this->all_annotation[$key]) > 0) {
            foreach ($this->all_annotation[$key] as $string) {
                if ($string !== "") {
                    $stringArr = explode(" ", $string);
                    $params[$stringArr[0]] = isset($stringArr[1]) ? $stringArr[1] : "";
                }
            }
        }
        return $params;
...
/**
 * Auth interface required to be implemented
 * By all auth models which
 */
interface AuthInterface {

    /**
     * validate method needs to return
     * boolean value
     */
...
/**
 * Class to store api session data
 * this class will be used for passing data from
 * auth method to implementation layer
 */
class ApiSession {
    ...
    /**
     * Restrict instance of this class
     */
    private function __construct() {
        //instance of this class is not allowed;
    }
    ...
    public static function getData($key = false) {
        ...
        if (array_key_exists($key, self::$session_data)) {
...
class InputValidation {
    ...
    public $basic_tags = '<br><p><a><strong><b><i><em><img><blockquote><code><dd><dl>
                        <hr><h1><h2><h3><h4><h5><h6><label><ul><li><span><sub><sup>';
     ...
     * @param array $validators The InputValidation validators
     *
     ...
     * Usage:
     *
     * InputValidation->set_field_names(array(
     * 	"name" => "My Lovely Name",
     ...
     * @return string
     */
    public function get_readable_errors($convert_to_string = false, $field_class = 'error-field', $error_class = 'error-message') {
